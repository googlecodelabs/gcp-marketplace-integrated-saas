...
public class ProcurementMessageReceiver implements MessageReceiver {
  ...
  public boolean processEntitlement(
    ...
    if (entitlement == null) {
      // Do nothing. The entitlement has to be canceled to be deleted, so this has already been
      // handled by a cancellation message.
      return true;
    }
    ...
    switch (eventType) {
      ...
      case ENTITLEMENT_PENDING_CANCELLATION:
        // Do nothing. We want to cancel once it's truly canceled. For now it's just set to not
        // renew at the end of the billing cycle.
        return true;
      case ENTITLEMENT_CANCELLATION_REVERTED:
        // Do nothing. The service was already active, but now it's set to renew automatically at
  ...
  // Cancels an existing customer in the database's service due to a canceled entitlement.
  private void cancelCustomer(Customer customer, Entitlement entitlement) throws IOException {
    // Clear out our records of the customer's plan.
    if (customer.products.containsKey(entitlement.getProduct())) {
      customer.products.remove(entitlement.getProduct());
      db.write(customer.procurementAccountId, customer);
    }
  }
